...
function makeRoomCode() {
  // 4 digit code, easy to type on a phone
...
io.on('connection', (socket) => {
  ...
  // Clock sync: reply as fast as possible with the server's current time
  socket.on('sync-ping', (clientSendTime, cb) => {
    cb({ clientSendTime, serverTime: Date.now() });
  });

  // Host uploads the audio file (sent as raw ArrayBuffer)
  ...
  // Anyone late-joining after audio was already uploaded can request it
  ...
  // Host broadcasts play/pause/seek commands with a target SERVER time
  socket.on('play-at', ({ roomCode, targetServerTime, offsetSeconds }) => {
    io.to(roomCode).emit('play-at', { targetServerTime, offsetSeconds });
  });
